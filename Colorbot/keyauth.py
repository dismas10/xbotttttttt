...
class api:
    ...
    def __do_request(self, post_data):
        try:
            rq_out = s.post(
                "https://api.colorant.store/1.0/", data=post_data, timeout=30
            )
            return rq_out.text
        except requests.exceptions.Timeout:
            print("Request timed out")
        except requests.exceptions.RequestException:
            print("Error: Unable to connect to the server. Please check your internet connection or try using a vpn.")
            os._exit(1)
